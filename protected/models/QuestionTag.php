...
class QuestionTag extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('name, name_ascii', 'required'),
			array('subject_id, class_id', 'numerical', 'integerOnly'=>true),
			array('name, name_ascii', 'length', 'max'=>500),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, name, name_ascii, subject_id, class_id', 'safe', 'on'=>'search'),
		);
	}
	...
	public function findIn($val){
		$criteria=new CDbCriteria;
		$criteria->addInCondition('id', explode(',', $val));
