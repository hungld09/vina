...
class DetailquestionBkAction extends CAction{
    public function run(){
...
//                $url_avatar = IPSERVER.$subcriber->url_avatar;
            ...
            $subs = Subscriber::model()->findByPk($subscriberId);
            $result[0]['channel_type'] = $subs->channel_type;
            if($result[0]['level_id'] == 0 || $result[0]['level_id'] == null || $result[0]['level_id'] == ''){
                $result[0]['level_id'] = 1;
            }
            $levelUser = Subscriber::model()->findByPk($params['user_id']);
            if($result[0]['subscriber_id'] == $params['user_id'] || $levelUser->type == 2){
