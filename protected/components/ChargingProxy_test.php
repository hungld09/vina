...
class ChargingProxy_test
{
    ...
    public static function chargingRegister($msisdn, $service, $transaction_id, $promotion = false, $debit_amount = -1, $channel = CHANNEL_TYPE_WAP, $note = NULL)
    { //debit_amount == -1 -> == $service->price (truong hop promotion==0)
        $msisdn = CUtils::validatorMobile($msisdn, 0);
    ...
    public static function chargingPromotionRegister($user_name, $user_ip, $msisdn, $service, $transaction_id, $debit_amount = 0, $channel = CHANNEL_TYPE_WAP, $note = NULL)
    { //debit_amount == -1 -> == $service->price (truong hop promotion==0)
        $msisdn = CUtils::validatorMobile($msisdn, 0);
        $command = self::CHARGING_REGISTER;
        if ($note == NULL) {
        ...
        $response = VinaphoneController::charging2($user_name, $user_ip, $transaction_id, $msisdn, $promotion, $debit_amount, $original_price, $command, $content, $channel);
        Yii::log("chargingRegister: error_code = " . $response->return);
        return $response->return;
    }

    public static function chargingRegister2($user_name, $user_ip, $msisdn, $service, $transaction_id, $promotion = false, $debit_amount = -1, $channel = CHANNEL_TYPE_WAP, $note = NULL)
    { //debit_amount == -1 -> == $service->price (truong hop promotion==0)
        Yii::log("chargingRegister: msisdn = " . $msisdn);
        $msisdn = CUtils::validatorMobile($msisdn, 0);
        $command = self::CHARGING_REGISTER;
        if ($note == NULL) {
            $content = $service->code_name;
        ...
        $response = VinaphoneController::charging2($user_name, $user_ip, $transaction_id, $msisdn, $promotion, $debit_amount, $original_price, $command, $trial = null, $bundle = null, $content, $channel);
        Yii::log("chargingRegister: error_code = " . $response->return);
        return $response;
    }
    ...
    public static function chargingCancel($user_name, $user_ip, $msisdn, $service, $transaction_id, $channel = CHANNEL_TYPE_WAP)
    {
        $msisdn = CUtils::validatorMobile($msisdn, 0);
        $command = self::CHARGING_CANCEL;
        $content = $service->code_name;
        $response = VinaphoneController::charging2($user_name, $user_ip, $transaction_id, $msisdn, false, 0, 0, $command, $trial=null, $bundle=null, $content, $channel);
        Yii::log("chargingCancel: error_code = " . $response->error);
    ...
    public static function processPayVod($channel_type, $vod, $subscriber)
    {
        ...
        if ($subscriber->status == Subscriber::STATUS_WHITE_LIST) { //xu ly truong hop white_list
            ...
            $chargingResult = ChargingProxy_test::chargingPayVod($msisdn, $vod, $subscriberTransaction->id, false, intval($subscriberTransaction->cost), $channel_type);
        }
    ...
    public static function processSubService($channel_type, $action, $service, $subscriber, $ssm = null, $user_name = NULL, $user_ip = NULL)
    {
        ...
        $msisdn = $subscriber->subscriber_number;
        Yii::log("msisdn = " . $msisdn . "\n");
        Yii::log("action = $action\n");
        if ($action == self::ACTION_REGISTER) {
            if ($usingServices != null) { //dang co ssm active, thong bao loi da dung d/vu roi
                ...
                if (isset($service)) {
                   ...
                   Yii::log("newTransaction processSubService \n");
...
//                    Yii::app()->session['requestid'] = $subscriberTransaction->id;
//                    if ($subscriber->status == Subscriber::STATUS_WHITE_LIST) { //xu ly truong hop white_list
//                        $chargingResult = CPS_OK;
//                    } else {
//                        if (count($subscriber->serviceSubscriberMappings) == 0 || (!(ServiceSubscriberMapping::isRegisteredInThreeMonth($subscriber->id)))) {
//                            Yii::log("ChargingProxy_test::chargingPromotionRegister \n");
//                            $freeFor1stTime = true;
////                            $subscriberTransaction->cost = 0;
//                            $chargingResult = ChargingProxy_test::chargingPromotionRegister($user_name, $user_ip, $msisdn, $service, time());
//                        } else {
                            Yii::log("ChargingProxy_test::chargingRegister2 \n");
                            $c_response = ChargingProxy_test::chargingRegister2($user_name, $user_ip, $msisdn, $service, time(), $promotion = null, intval($service->price), $channel_type, $trial=null, $bundle=null, $note=null);
                            $chargingResult = $c_response->error;
                            Yii::log("ChargingProxy_test::chargingRegister2 chargingResult = $chargingResult\n");
                            ...
                            //$chargingResult = ChargingProxy_test::chargingRegister($msisdn, $service, $subscriberTransaction->id, false, intval($subscriberTransaction->cost), $channel_type);
//                        }
//                    }
...
////                    $subscriberTransaction->save();
            ...
            if ($usingServices != null) {
                ...
                if ($service->code_name == $service->code_name) { //kiem tra huy dung dich vu dang dung ko
                    $chargingResult = $subscriber->cancelService2($usingServices, $channel_type, -1, "", $user_name, $user_ip);
                    if ($chargingResult == CPS_OK || $chargingResult == CPS_OK_6) {
            ...
            if ($subscriber->status == Subscriber::STATUS_WHITE_LIST) { //xu ly truong hop white_list
                ...
                $chargingResult = ChargingProxy_test::chargingExtendService($msisdn, $service, $recurTransaction->id, $channel_type);
            }
    ...
    public static function confirmGift($subscriber, $usingService, $code)
    {
        ...
        */
        if (($usingService == NULL) || ($usingService != NULL && $usingService->using_days < $serviceGift->using_days)) {
            ...
            $chargingResult = ChargingProxy_test::chargingGift($sendSubscriber->subscriber_number, $serviceGift, 0, intval($subscriberTransaction->cost));
            $isChargingSuccess = false;
            if ($chargingResult == CPS_OK) {
                $isChargingSuccess = true;
