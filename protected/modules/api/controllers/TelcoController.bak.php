...
// 	http://vfilm.vn/api/telco/getTransactions?requestid=1&msisdn=84916731158&note=vnp&application=vnp_api&channel=api&username=chau&userip=127.0.0.1&fromdate=20131110&todate=20131120
...
// 			$root->appendChild($xmlDoc->createElement("phone_number", $this->msisdn));
			...
			$root->appendChild($xmlDoc->createElement("error_desc", "Lấy danh sách giao dịch thành công"));
...
/* 
	...
	3.1
	http://vfilm.vn/api/telco/registerService?requestid=1&msisdn=84916731158&packagename=VFILM&promotion=1w&trial=1w&bundle=1&note=vnp&application=vnp_api&channel=api&username=chau&userip=127.0.0.1 */
...
// 			Yii::log("*** 7 *** ".microtime());
			...
			if($chargingResult == CPS_OK) {
				...
				$trans->status = 1;
// 				if($application != 'VASVOUCHER') {
					$mtMessage = $this->getMTContent(self::ACTION_REGISTER, $application, $isFirstUse);
...
// 			Yii::log("*** 9 *** ".microtime());
			...
			if($ssm == NULL) {
				$ssm = $subscriber->addService($service, null, USING_TYPE_REGISTER, null);
			}
// 			Yii::log("*** 10 *** ".microtime());
			...
			if(isset($trial)) {
				if($trial_days > 0) {
					$continueCheck = false;
					$ssm->expiry_date = date('Y-m-d H:i:s', time() + $trial_days*86400);
					$ssm->update();
				}
			}
			if($continueCheck) {
				if(isset($promotion)) {
					if($using_days > 0) {
						$ssm->expiry_date = date('Y-m-d H:i:s', time() + $using_days*86400);
	...
	private function getMTContent($action, $application, $isFirstUse = false) { //tuy theo action & application goi api ma co noi dung MT khac nhau
		switch($action) {
...
// 					MOBILE_ADS hoặc VASPORTAL thì hoặc VASDEALER:
					...
					case self::APPLICATION_VASDEALER:
						if($isFirstUse) {
							$mtMessage = "Chuc mung Quy Khach da dang ky thanh cong dich vu vFilm cua VinaPhone. Quy Khach duoc mien phi 7 ngay trai nghiem dich vu vFilm. Sau khi het thoi han mien cuoc, VinaPhone se tinh cuoc dich vu 10.000d/tuan va dich vu se duoc tu dong gia han. Quy Khach vui long truy cap http://vfilm.vn/ de thuong thuc hang nghin bo phim hap dan. De huy dich vu, Quy Khach vui long soan HUY gui 1579.";
						}
						else {
							$mtMessage = "Chuc mung Quy Khach da dang ky thanh cong goi dich vu vFilm cua VinaPhone. Quy Khach vui long truy cap http://vfilm.vn/ de thuong thuc hang nghin bo phim hap dan. Gia cuoc 10.000d/tuan, dich vu se duoc tu dong gia han hang tuan. De huy dich vu, Quy Khach vui long soan HUY gui 1579.";
						}
						break;
					case self::APPLICATION_CCOS:
						if($isFirstUse) {
							$mtMessage = "Chuc mung Quy Khach da dang ky thanh cong dich vu vFilm qua he thong Cham soc khach hang cua VinaPhone. Quy Khach duoc mien phi 7 ngay trai nghiem dich vu vFilm. Sau khi het thoi han mien cuoc, VinaPhone se tinh cuoc dich vu 10.000d/tuan va dich vu se duoc tu dong gia han. Quy Khach vui long truy cap http://vfilm.vn/ de thuong thuc hang nghin bo phim hap dan. De huy dich vu, Quy Khach vui long soan HUY gui 1579.";
						}
						else {
							$mtMessage = "Chuc mung Quy Khach da dang ky thanh cong dich vu vFilm qua he thong Cham soc khach hang cua VinaPhone. Quy Khach vui long truy cap http://vfilm.vn/ de thuong thuc hang nghin bo phim hap dan. Gia cuoc 10.000d/tuan, dich vu se duoc tu dong gia han. De huy dich vu, Quy Khach vui long soan HUY gui 1579.";
						}
						break;
					default:
						if($isFirstUse) {
...
// 	3.3
// 	http://vfilm.vn/api/telco/cancelService?requestid=2&msisdn=84916731158&note=vnp&application=vnp_api&channel=api&username=chau&userip=127.0.0.1
...
// 	http://vfilm.vn/api/telco/getSubscriberInfo?requestid=1&msisdn=84916731158&packagename=VFILM&note=vnp&application=vnp_api&channel=api&username=chau&userip=127.0.0.1
...
// 	http://vfilm.vn/api/telco/getSubscriberAllPackageInfo?requestid=1&msisdn=84916731158&note=vnp&application=vnp_api&channel=api&username=chau&userip=127.0.0.1
...
// 	http://vfilm.vn/api/telco/CancelSubscriber?requestid=1&msisdn=84916731158&application=vnp_api&channel=api&username=chau&userip=127.0.0.1
...
// 	http://vfilm.vn/api/telco/ChangeMsisdn?requestid=1&msisdnA=84916731158&msisdnB=84916731159&application=vnp_api&channel=api&username=chau&userip=127.0.0.1
...
// 	http://vfilm.vn/api/telco/GetLastAction?requestid=1&msisdn=84916731158&note=vnp&application=vnp_api&channel=api&username=chau&userip=127.0.0.1&fromdate=20131110&todate=20131122
