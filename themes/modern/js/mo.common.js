...
$(document).ready(function() {
    ...
    MoNav.init();
    MoNumber.init();
    if ($('.fn-expand').length) $('.fn-expand').click(function() {
        var box = $(this).attr('expand-box'),
            css = $(this).attr('expand-class'),
            des = $(this).attr('expand-desc'),
            html = $(this).html();
    ...
    if ($('.fn-tab').length) $('.fn-tab').click(function() {
    ...
    if (sel.length > 0) sel.on('change', function(e) {
...
var MoAutoLoad = function(con) {
    if (con.attr('id')) this.conId = con.attr('id');
        ...
        var d = new Date();
        this.conId = 'fnAuotLoad' + d.getMilliseconds();
            ...
            if (k) {
                ...
                for (var i = 0; i < kHistory.length; i++) {
                    if (kHistory[i].trim().toLowerCase().search(k) != -1) {
                        items += '<li><a href="'+MO.MO_URL+'/mphim/video/search?q=' + encodeURI(kHistory[i]) + '">' + kHistory[i] + '</a></li>';
                    }
                }
            } else {
                for (var i = 0; i < kHistory.length; i++) {
...
var MoLike = {
    ...
    pushLike: function(tar) {
        var _this = this;
        var url = '',
            uid = tar.attr('like-uid'),
            token = tar.attr('like-token'),
            key = tar.attr('like-key');
        var unlike = true;
        ...
        function(like) {
            if (typeof like === 'object' && !like.error) {
                if (unlike) {
                    tar.attr('like-like', false);
                    _this.changeLike(-1);
                } else {
                    tar.attr('like-like', true);
                    _this.changeLike(1);
                }
            }
        });
    ...
    stats: function(fn) {
        ...
        $.getJSON('http://like.mp3.zing.vn/get?uid=' + o.attr('like-uid') + '&sid=' + o.attr('like-key') + '&token=' + o.attr('like-token') + '&callback=?', function(data, status, xhr) {
            if (status == 'success') {
                ...
                if (data.like) $('#fnLike').attr('like-like', true);
...
var MoDevice = {
    ...
    setCookie: function(c_name, value, exdays) {
        ...
        var c_value = escape(value) + ((exdays == null) ? "" : "; expires=" + exdate.toUTCString()) + ";domain=.mphim.vn;path=/";
